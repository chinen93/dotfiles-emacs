...
#+begin_src emacs-lisp
  (when (featurep 'elgrep)
    ...
    (defun my--elgrep-common (dir regex)
      "Run elgrep on DIR searching for REGEX"
      (let* ((options (list :recursive t
                            :interactive t
                            :depth 0))
             (options (elgrep-intern-plist-keys options))
             (file-name-re "\.org")
  ...
  (interactive
   (let* ((dir (file-name-nondirectory (directory-file-name my/config-notes-folder)))
          (prompt (format "Search on %s - Emacs regexp: " dir)))
     (list (read-regexp prompt nil))))
  (let* ((dir (directory-file-name my/config-notes-folder)))
    (my--elgrep-common dir regex)))


    (defun my-elgrep-from-here (regex)
  "Search current directory for REGEX."
  (interactive
