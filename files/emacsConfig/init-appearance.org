...
#+begin_src emacs-lisp
  ;; Variable to hold the name of the DARK theme
  (defvar my/theme-dark-name 'solarized-dark-high-contrast
    "The name of the dark theme to use")

  ;; Variable to hold the name of the LIGHT theme
  (defvar my/theme-light-name 'solarized-light-high-contrast
    "The name of the light theme to use")

  ;; Variable to hold the current state (A symbol: 'dark' or 'light')
  (defvar my/theme-current-state 'dark
    "Tracks whether the current theme is :dark or :light.")

  ;; Function to toggle between the two modes
  (defun my-toggle-theme ()
    "Toggles the theme between the configured dark and light modes.
     Switches the theme and updates the tracking variable."
    (interactive)
    (let ((current-state my/theme-current-state))
      (if (string-equal current-state 'dark)
          ;; If currently dark, switch to light
          (progn
            (message "Switching from DARK to LIGHT theme")
  	  (load-theme my/theme-light-name t)
            (setq my/theme-current-state 'light))
        ;; If currently light, switch to dark
        (progn
          (message "Switching from LIGHT to DARK theme")
  	(load-theme my/theme-dark-name t)
          (setq my/theme-current-state 'dark))))
    (message "Theme switched. Current state: %s" (if (string-equal my/theme-current-state 'dark) "DARK" "LIGHT")))
