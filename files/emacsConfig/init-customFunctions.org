...
*** Functions that Do Not Change Buffer
:PROPERTIES:
:ID:       9ffe670a-b965-4862-8bf7-847df522c827
:END:

Some functions don't need to change the calling buffer.

#+BEGIN_SRC emacs-lisp
  (define-key my/keymap (kbd "M-e") 'my-function-menu)
...
*** Function that Change Buffer
:PROPERTIES:
:ID:       8f9dcb2c-f189-4c5e-b87f-737552939ff4
:END:

Some functions need to change the calling buffer

**** Macro to Change Buffer
:PROPERTIES:
:ID:       573f3151-94f7-440e-bced-438c52dc077a
:END:

Create a macro to go back to the calling buffer, so that any change can happen without problem.

#+BEGIN_SRC emacs-lisp
  (defmacro my-macro-iteractive-function (body)
    "Execute BODY in the correct buffer, AKA buffer where menu was called"
    `(lambda ()
       (interactive)
       (select-window my/window-call-menu)

       ,body
       (delete-window (get-buffer-window my/buffer-menu-name))
       (setq my/buffer-call-menu nil)
       (setq my/window-call-menu nil)))

#+END_SRC

**** Functions
:PROPERTIES:
:ID:       1c34df61-dbc7-4d02-ac53-10b7dd9225a2
:END:

Functions that change the calling buffer.

#+BEGIN_SRC emacs-lisp
  (define-key start-menu-mode-map (kbd "T")
    (my-macro-iteractive-function (insert "TESTE")))
...
#+BEGIN_SRC emacs-lisp
  (defun my-function-menu ()
    "Make Emacs like MS Windows."
    (interactive)

    ;;(split-window-right)
    (setq my/buffer-call-menu (buffer-name))
    (setq my/window-call-menu (get-buffer-window my/buffer-call-menu))

    (switch-to-buffer-other-window (get-buffer-create my/buffer-menu-name))
