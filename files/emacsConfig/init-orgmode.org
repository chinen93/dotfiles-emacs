...
#+BEGIN_SRC emacs-lisp
  ...
  (setq org-todo-keywords
        '((sequence "TODO(t)" "WORKING(w!)" "WEEK(W!)" "|" "DONE(d!)")
          (sequence "|" "CANCELED(c@/!)")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'org-agenda-files
               (concat my/dropbox-folder "Notes/Datas.org"))
     ...
     (
      (tags "ativo+PRIORITY=\"A\""
            ((org-agenda-overriding-header "Objetivos Primários para o ano")
             (org-agenda-files `(,(concat my/dropbox-folder "Notes/Planos.org")))))
      ...
      (todo "TODO"
            ((org-agenda-overriding-header "\nPlanejamento Semanal\n")
             (org-agenda-files
              `(,(concat my/dropbox-folder "Notes/PlanejamentoSemanal.org")))))
      )
     nil))
#+END_SRC
