...
#+BEGIN_SRC emacs-lisp
  ;; Set Org folder
  (defvar my/org-folder "~/org/"
    "Org directory")

  ;; Only set this org variable if there no other variable for the dropbox folder
  (when (file-directory-p my/dropbox-folder)
    (setq my/org-folder my/dropbox-folder))
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'org-agenda-files
               (concat my/org-folder "Organizador.org"))

  (add-to-list 'org-agenda-files
               (concat my/org-folder "Notes/Projetos.org"))

  (add-to-list 'org-agenda-files
               (concat my/org-folder "Contacts.org"))

  (add-to-list 'org-agenda-files
               (concat my/org-folder "Notes/Pedro.org"))
...
#+BEGIN_SRC emacs-lisp
  (defvar my/ledger-transaction-template
    (concat
     "%^{YYYY}/%^{MM}/%^{DD} Dinheiro
      Expenses:Dinheiro                      R$ %^{50.00}
      Assets:Bank:Caixa")
    "Template for generic transaction")

  (add-to-list
   'org-capture-templates
   `("D" "Ledger Dinheiro" entry (file "~/Dropbox/Ledger.ledger")
     ,my/ledger-transaction-template
     :empty-lines 1))
#+END_SRC
